...
const useAuth = () => {
    ...
    const proveAuth = async () => {
        try {
            const response = await axios.post("https://ec26-2001-871-25f-1b02-115-8b33-1eb1-e220.ngrok-free.app/google/proveAuth")
            console.log(response.data)
            setStatus(response.data.status0)
            if (response.data.message == "No auth token or email found") {
                // redirect to sign in
                window.location.href = "https://ec26-2001-871-25f-1b02-115-8b33-1eb1-e220.ngrok-free.app/signin"
            }
